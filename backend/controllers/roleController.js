...
export const fetchRoles = async (req, res) => {
  ...
  try {
    const [roles] = await optramisDB.query(
...
export const createRole = async (req, res) => {
  const role = req.body;
  try {
    const [result] = await optramisDB.query("INSERT INTO projectroles SET ?", [role]);
    // Fetch the inserted role using insertId
    const [rows] = await optramisDB.query("SELECT * FROM projectroles WHERE id = ?", [
      result.insertId,
    ]);
...
export const updateRole = async (req, res) => {
  const  role  = req.body;
  try {
    const [result] = await optramisDB.query(
...
export const fetchPermissions = async (req, res) => {
  try {
    const [permissions] = await optramisDB.query(
...
export const createPermission = async (req, res) => {
  ...
  try {
    const [result] = await optramisDB.query("INSERT INTO permissions SET ?", [permission]);
...
export const updatePermission = async (req, res) => {
  ...
  try {
    ...
    const [rows] = await optramisDB.query("SELECT * FROM permissions WHERE id = ?", [
      permissison.id,
    ]);
